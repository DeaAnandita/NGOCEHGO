    ...
    <div class="flex">
        ...
        <div class="flex-1 py-6 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-xl font-bold text-gray-800 mb-6">Tambah Data Konflik Sosial</h3>

                <form action="{{ route('keluarga.konfliksosial.store') }}" method="POST">
                    @csrf
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach($masterKonflik as $konflik)
                                <div class="bg-gray-50 p-3 rounded-lg border border-gray-200">
                                    ...
                                    <select name="konfliksosial_{{ $konflik->kdkonfliksosial }}"
                                        class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                        <option value="">-- Silahkan Pilih --</option>
                                        @foreach($masterJawab as $jawab)
                                            <option value="{{ $jawab->kdjawabkonflik }}">{{ $jawab->jawabkonflik }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            @endforeach
