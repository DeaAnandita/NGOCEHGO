    ...
    <div class="flex">
        ...
        <div class="flex-1 py-6 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-xl font-bold text-gray-800 mb-6">Tambah Data Aset Lahan</h3>

                <form action="{{ route('keluarga.asetlahan.store') }}" method="POST">
                    @csrf
                                ...
                                <div>
                                    ...
                                    <select name="{{ $field }}" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                        ...
                                        @foreach($masterJawab as $jawab)
                                            <option value="{{ $jawab->kdjawablahan }}" {{ old($field) == $jawab->kdjawablahan ? 'selected' : '' }}>
                                                {{ $jawab->jawablahan }}
                                            </option>
                                        @endforeach
                                    </select>
