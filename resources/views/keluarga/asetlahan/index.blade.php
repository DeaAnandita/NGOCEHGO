                                ...
                                @forelse($asetlahans as $aset)
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        @for ($i = 1; $i <= 10; $i++)
                                            <td class="border border-gray-200 px-4 py-4">
                                                <span class="text-xs px-2 py-1 rounded-full
                                                    {{ $aset->{"asetlahan_$i"} == 0 ? 'bg-gray-100 text-gray-600' : 'bg-green-100 text-green-800' }}">
                                        ...
                                        <td class="border border-gray-200 px-4 py-4 text-center space-x-3">
                                            ...
                                            <form action="{{ route('keluarga.asetlahan.destroy', $aset->no_kk) }}" method="POST" class="inline">
                                                @csrf @method('DELETE')
