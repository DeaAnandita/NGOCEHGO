            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        <!-- Dropdown Per Page -->
                        <form method="GET" action="{{ route('keluarga.asetternak.index') }}" class="flex items-center gap-2">
                        ...
                        <!-- Form Search -->
                        <form method="GET" action="{{ route('keluarga.asetternak.index') }}" class="flex items-center">
                ...
                <div class="relative">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-gray-700">
                                @forelse($asetternaks as $aset)
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        @for ($i = 1; $i <= 24; $i++)
                                            <td class="border border-gray-200 px-4 py-4">{{ $aset->{"asetternak_$i"} ?? '-' }}ekor</td>
                                        @endfor
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-center mt-4 gap-2 text-sm text-gray-600">
                        <div>
                            Menampilkan {{ $asetternaks->firstItem() }} - {{ $asetternaks->lastItem() }} dari {{ $asetternaks->total() }} data
                        </div>
