    ...
    <div class="flex">
        ...
        <div class="flex-1 py-6 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-xl font-bold text-gray-800 mb-6">Edit Data Aset Ternak</h3>

                <form action="{{ route('keluarga.asetternak.update', $asetternak->no_kk) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div>
                                ...
                                <select name="no_kk" id="no_kk"
                                    ...
                                    @foreach($keluargas as $kel)
                                        <option value="{{ $kel->no_kk }}" {{ old('no_kk', $asetternak->no_kk) == $kel->no_kk ? 'selected' : '' }}>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach($masterAset as $index => $aset)
                                @php $field = 'asetternak_' . ($index + 1); @endphp
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">{{ $aset->asetternak }}</label>
                                    <input type="text" name="{{ $field }}"
                                           value="{{ old($field, $asetternak->$field) }}"
