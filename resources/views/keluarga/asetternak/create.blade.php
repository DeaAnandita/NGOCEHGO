    ...
    <div class="flex">
        ...
        <div class="flex-1 py-6 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-xl font-bold text-gray-800 mb-6">Tambah Data Aset Ternak</h3>
                ...
                <form action="{{ route('keluarga.asetternak.store') }}" method="POST">
                    @csrf
                    ...
                    <!-- Detail Aset Ternak -->
                    <div class="mb-8">
                        <h4 class="text-sm font-semibold text-gray-700 mb-4">Detail Aset Ternak</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach($masterAset as $aset)
                                <div>
                                    <label class="block text-sm font-medium text-gray-700">{{ $aset->asetternak }} (ekor)</label>
                                    <input
                                        type="number"
                                        name="asetternak_{{ $aset->kdasetternak }}"
                                        class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                        min="0"
                                        value="{{ old('asetternak_'.$aset->kdasetternak, 0) }}"
                                        placeholder="0"
                                    >
                                    @error('asetternak_'.$aset->kdasetternak)
                                        <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                                    @enderror
                                </div>
                            @endforeach
                    ...
                    <div class="mt-6 flex space-x-4 justify-end">
                        ...
                        <a href="{{ route('keluarga.asetternak.index') }}" class="bg-gray-200 text-gray-700 px-6 py-2.5 text-sm font-medium rounded-lg hover:bg-gray-300 transition duration-200 shadow-sm">Batal</a>
                    </div>
                </form>
            </div>
        </div>
