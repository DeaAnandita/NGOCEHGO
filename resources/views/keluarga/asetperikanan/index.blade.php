            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <!-- Form Search -->
                        <form method="GET" action="{{ route('keluarga.asetperikanan.index') }}" class="flex items-center">
                ...
                <div class="relative">
                    <div class="overflow-x-auto w-full">
                        <table id="asetPerikananTable" class="w-full table-auto border-collapse text-sm">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-gray-700">
                                @forelse($asetperikanans as $aset)
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="border border-gray-200 px-4 py-3">{{ $asetperikanans->firstItem() + $loop->index }}</td>
                                        ...
                                        @for ($i = 1; $i <= 6; $i++)
                                            <td class="border border-gray-200 px-4 py-4 text-center">{{ $aset->{"asetperikanan_$i"} ?? '-' }} unit</td>
                                        @endfor
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-center mt-4 gap-2 text-sm text-gray-600">
                        <div>
                            Menampilkan {{ $asetperikanans->firstItem() }} - {{ $asetperikanans->lastItem() }} dari {{ $asetperikanans->total() }} data
                        </div>
                        <div class="w-full sm:w-auto">
                            {{ $asetperikanans->appends(['search' => request('search'), 'per_page' => request('per_page')])->links() }}
                        </div>
                    </div>
                </div>
            </div>
