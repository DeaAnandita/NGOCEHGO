            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="relative">
                        ...
                        <table id="asetPerikananTable" class="min-w-full table-fixed border-collapse text-sm">
                            <thead class="bg-blue-50 text-gray-600 uppercase text-xs font-semibold sticky top-0 z-10">
                                <tr>
                                    ...
                                    @for ($i = 1; $i <= 6; $i++)
                                        <th class="border border-gray-200 px-4 py-3 w-[120px] text-left">
                                            <span title="{{ $masterAset[$i] ?? '' }}">
                                                {{ Str::limit(Str::replace('Memiliki ', '', $masterAset[$i] ?? ''), 20) }}
                                            </span>
                                        </th>
                                    @endfor
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-gray-700">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="12" class="border border-gray-200 px-4 py-4 text-center text-gray-500">
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-center mt-4 gap-2 text-sm text-gray-600">
                        <div>
                            Menampilkan {{ $asetperikanans->firstItem() ?? 0 }} - {{ $asetperikanans->lastItem() ?? 0 }} dari {{ $asetperikanans->total() ?? 0 }} data
                        </div>
