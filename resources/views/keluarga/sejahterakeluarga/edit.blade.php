    ...
    <div class="flex">
        ...
        <div class="flex-1 py-6 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-xl font-bold text-gray-800 mb-6">Edit Data Sejahtera Keluarga</h3>

                <form action="{{ route('keluarga.sejahterakeluarga.update', $sejahterakeluarga->no_kk) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div>
                                ...
                                <select name="no_kk" id="no_kk"
                                    ...
                                    @foreach($keluargas as $kel)
                                        <option value="{{ $kel->no_kk }}" {{ old('no_kk', $sejahterakeluarga->no_kk) == $kel->no_kk ? 'selected' : '' }}>
                    ...
                    <!-- Detail Sejahtera Keluarga -->
                    <div class="mb-8">
                        <h4 class="text-sm font-semibold text-gray-700 mb-4">Detail Sejahtera Keluarga</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach($masterPembangunan as $pembangunan)
                                @php
                                    $field = 'sejahterakeluarga_' . $pembangunan->kdpembangunankeluarga;
                                    $value = old($field, $sejahterakeluarga->$field);
                                @endphp
                                <div>
