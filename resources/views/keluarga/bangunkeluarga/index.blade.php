                        ...
                        <table class="min-w-full table-fixed border-collapse text-sm">
                            <thead class="bg-blue-50 text-gray-600 uppercase text-xs font-semibold sticky top-0 z-10">
                                <tr>
                                    ...
                                    @isset($masterPembangunankeluarga)
                                        @foreach($masterPembangunankeluarga as $pembangunankeluarga)
                                            <th class="border border-gray-200 px-4 py-3 w-32 text-left">
                                                <span title="{{ $pembangunankeluarga->pembangunankeluarga }}">
                                                    {{ Str::replace('Keluarga', '', $pembangunankeluarga->pembangunankeluarga) }}
                                                </span>
                                            </th>
                                        @endforeach
                                    @endisset
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-gray-700">
                                @forelse($bangunkeluargas as $data)
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        @foreach($masterPembangunankeluarga as $pembangunankeluarga)
                                            @php 
                                                $field = 'bangunkeluarga_' . $pembangunankeluarga->kdpembangunankeluarga;
                                ...
                                @empty
                                    <tr>
                                        <td colspan="{{ 3 + (isset($masterPembangunankeluarga) ? count($masterPembangunankeluarga) : 0) + 1 }}" 
