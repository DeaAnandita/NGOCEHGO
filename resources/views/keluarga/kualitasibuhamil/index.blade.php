            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
                    <div class="flex items-center gap-3 flex-wrap">
                        ...
                        <div x-data="{ open: false }" class="relative">
                            ...
                            <div x-show="open" @click.away="open = false"
                                ...
                                <a href="{{ route('export.kualitasibuhamil') }}"
                                ...
                                <a href="{{ route('kualitasibuhamil.exportAnalisisPDF') }}"
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 w-full sm:w-auto">
                        ...
                        <!-- Dropdown Per Page -->
                        <form method="GET" action="{{ route('keluarga.kualitasibuhamil.index') }}" class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                        ...
                        <!-- Form Search -->
                        <form method="GET" action="{{ route('keluarga.kualitasibuhamil.index') }}" 
                ...
                <div class="relative">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200 text-gray-700">
                                @forelse($kualitasibuhamils as $ibu)
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="border border-gray-200 px-4 py-3">{{ $kualitasibuhamils->firstItem() + $loop->index }}</td>
                                        ...
                                        <td class="border border-gray-200 px-2 py-2 text-center w-[80px]">
                                            <div class="flex justify-center gap-1">
                                                ...
                                                <form action="{{ route('keluarga.kualitasibuhamil.destroy', $ibu->no_kk) }}" method="POST"
                    ...
                    <div class="flex flex-col sm:flex-row justify-between items-center mt-4 gap-2 text-sm text-gray-600">
                        <div>
                            Menampilkan {{ $kualitasibuhamils->firstItem() ?? 0 }} - {{ $kualitasibuhamils->lastItem() ?? 0 }} dari {{ $kualitasibuhamils->total() ?? 0 }} data
                        </div>
                        <div class="w-full sm:w-auto">
                            {{ $kualitasibuhamils->appends(['search' => request('search'), 'per_page' => request('per_page')])->links() }}
                        </div>
                    </div>
                </div>
                </div>
