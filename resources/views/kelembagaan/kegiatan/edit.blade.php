    ...
    <div class="flex">
        ...
        <div class="flex-1 py-6 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <form action="{{ route('kelembagaan.kegiatan.update', $kegiatan->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                                ...
                                <select name="kdjenis" class="w-full rounded-lg border-gray-300" required>
                                    @foreach ($jenis as $j)
                                        <option value="{{ $j->kdjenis }}"
                                            {{ old('kdjenis', $kegiatan->kdjenis) == $j->kdjenis ? 'selected' : '' }}>
                                            {{ $j->jenis_kegiatan }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="kdunit" class="w-full rounded-lg border-gray-300" required>
                                    @foreach ($unit as $u)
                                        <option value="{{ $u->kdunit }}"
                                            {{ old('kdunit', $kegiatan->kdunit) == $u->kdunit ? 'selected' : '' }}>
                                            {{ $u->unit_keputusan }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="kdperiode" class="w-full rounded-lg border-gray-300" required>
                                    @foreach ($periode as $p)
                                        <option value="{{ $p->kdperiode }}"
                                            {{ old('kdperiode', $kegiatan->kdperiode) == $p->kdperiode ? 'selected' : '' }}>
                                            {{ $p->tahun_awal }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="kdstatus" class="w-full rounded-lg border-gray-300" required>
                                    @foreach ($status as $s)
                                        <option value="{{ $s->kdstatus }}"
                                            {{ old('kdstatus', $kegiatan->kdstatus) == $s->kdstatus ? 'selected' : '' }}>
                                            {{ $s->status_kegiatan }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="kdsumber" class="w-full rounded-lg border-gray-300" required>
                                    @foreach ($sumber as $s)
                                        <option value="{{ $s->kdsumber }}"
                                            {{ old('kdsumber', $kegiatan->kdsumber) == $s->kdsumber ? 'selected' : '' }}>
                                            {{ $s->sumber_dana }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <select name="keputusan_id" class="w-full rounded-lg border-gray-300">
                                    ...
                                    @foreach ($keputusan as $k)
                                        <option value="{{ $k->id }}"
                                            {{ old('keputusan_id', $kegiatan->keputusan_id) == $k->id ? 'selected' : '' }}>
