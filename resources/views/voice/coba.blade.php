<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto py-8 px-6 space-y-6">
        ...
        <div class="mt-4">
            <button id="bypassValidation"
                class="px-6 py-2 bg-yellow-500 text-white rounded-lg shadow hover:bg-yellow-600">
                Lewati Validasi Suara (Testing Saja)
            </button>
        </div>

        <div id="inputArea" class="bg-white rounded-2xl shadow-lg p-6">
            <h2 id="modulTitle" class="text-2xl font-bold text-center mb-6 text-green-800">Input Data Penduduk via Suara
            </h2>
    ...
    <script>
        ...
        const wilayahQuestions = [{
                type: "select",
                label: "Provinsi asalnya apa?",
                field: "kdprovinsi",
                options: provinsiOptions
            },
            {
                type: "select",
                label: "Kabupaten atau kota asalnya apa?",
                field: "kdkabupaten",
                dynamic: true,
                parentField: "kdprovinsi",
                dynamicUrl: "/voice/kabupaten/"
            },
            {
                type: "select",
                label: "Kecamatan asalnya apa?",
                field: "kdkecamatan",
                dynamic: true,
                parentField: "kdkabupaten",
                dynamicUrl: "/voice/kecamatan/"
            },
            {
                type: "select",
                label: "Desa atau kelurahan asalnya apa?",
                field: "kddesa",
                dynamic: true,
                parentField: "kdkecamatan",
                dynamicUrl: "/voice/desa/"
            }
        ];
        ...
        function injectWilayahIfNeeded() {
            ...
            if (answers.kdmutasimasuk === '3' && !hasWilayah) {
                ...
                questions[1].splice(idx + 1, 0, {
                    ...
                    dynamic: true,
                    parentField: "kdprovinsi",
                    dynamicUrl: "/voice/get-kabupaten/"
                }, {
                    type: "select",
                    label: "Kecamatan asalnya apa?",
                    field: "kdkecamatan",
                    dynamic: true,
                    parentField: "kdkabupaten",
                    dynamicUrl: "/voice/get-kecamatan/"
                }, {
                    type: "select",
                    label: "Desa atau kelurahan asalnya apa?",
                    field: "kddesa",
        ...
        function cleanOptionText(t) {
            return t.replace(/\//g, ' atau ').replace(/_/g, ' ').replace(/\-/g, ' ke ');
        }
        ...
        function findBestMatch(text, options) {
            ...
            Object.entries(options).forEach(([id, name]) => {
                ...
                let s = 0;
                if (nn.includes(n)) s = 1000;
                else if (n.includes(nn)) s = 800;
                else nn.split(' ').forEach(w => {
                    if (n.includes(w)) s += w.length * 3;
                });
                if (s > score) {
                    score = s;
                    best = [id, name];
        ...
        function mapPendapatanToCode(pendapatan) {
            ...
            if (input === '' ||
                /tidak ada|tidak berpendapatan|nol|tidak punya|tanpa pendapatan|belum bekerja|belum berpenghasilan/i.test(
                    input)) {
        ...
        function loadModulData(id) {
            const a = localStorage.getItem('voiceAnswersPenduduk');
            if (a) answers = JSON.parse(a);
            const s = localStorage.getItem('modulStatusPenduduk');
            if (s) modulStatus = JSON.parse(s);
        ...
        function updateProgressSteps() {
            ...
            modules.forEach((m, i) => {
                ...
                circle.className =
                    `w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold text-white ${modulStatus[m.id]==='completed'?'bg-blue-600':modulStatus[m.id]==='active'?'bg-green-600':'bg-gray-300 text-gray-600'}`;
                circle.textContent = m.id;
                const txt = document.createElement('div');
                txt.className = 'ml-2 text-left';
                txt.innerHTML =
                    `<div class="font-medium text-sm">${m.name}</div><div class="text-xs ${modulStatus[m.id]==='completed'?'text-blue-600':modulStatus[m.id]==='active'?'text-green-600':'text-gray-500'}">${modulStatus[m.id]==='completed'?'Selesai':modulStatus[m.id]==='active'?'Aktif':'Belum'}</div>`;
        ...
        function checkAllCompletedAndShowSimpanBtn() {
            ...
            if (allDone) {
                b.style.backgroundColor = '#14b8a6';
                b.disabled = false;
            } else {
                b.style.backgroundColor = '#9ca3af';
                b.disabled = true;
            }
        }
        ...
        async function renderQuestion() {
            ...
            if (q.type === "select") {
                ...
                Object.entries(q.options).forEach(([id, nama]) => {
                    ...
                    html +=
                        `<div class="option-card ${sel}" data-value="${id}" data-text="${nama}"><span class="text-sm font-medium">${nama}</span></div>`;
                });
                html += '</div>';
            } else {
                const val = answers[q.field] || '';
                html +=
                    `<div class="max-w-md mx-auto"><input type="text" id="inputAnswer" class="w-full border border-gray-300 rounded-xl p-3 text-center text-lg" readonly value="${val}" placeholder="Jawaban muncul di sini..."></div>`;
            }
        ...
        async function speakQuestionAndOptions() {
            ...
            // Hint pembuka modul
            if (currentModul === 1 && step === 0) await speak(
                "Modul Data Penduduk dimulai. Jawab pertanyaan satu per satu.");
            ...
            if (currentModul === 5 && step === 0) await speak(
                "Modul Program Serta. Jawab 1 untuk Ya, 2 pernah, 3 tidak.");
            if ([6, 7, 8].includes(currentModul) && step === 0) await speak(
                "Modul Lembaga. Jawab 1 ya, 2 PERNAH, 3 TIDAK.");
            ...
            if (
                ...
                !["kdprovinsi", "kdkabupaten", "kdkecamatan", "kddesa", "kdpekerjaan", "kdlapanganusaha"].includes(q
                    .field)
            ) {
        ...
        function showReviewForm() {
            ...
            if (currentModul === 1) {
                ...
                html +=
                    `<div><label class="block text-xs font-medium mb-1">Kewarganegaraan</label>
                ...
                // 20. Tanggal Mutasi
                html +=
                    `<div><label class="block text-xs font-medium mb-1">Tanggal Mutasi</label>
                ...
                if (mutasiSelect) {
                    mutasiSelect.addEventListener('change', function() {
                        ...
                        if (this.value === '3') {
                            ...
                            questions[1] = questions[1].filter(q => !["kdprovinsi", "kdkabupaten", "kdkecamatan",
                                "kddesa"
                            ].includes(q.field));
                        }
                        saveData();
                ...
                if (answers.kdprovinsi && answers.kdmutasimasuk === '3') {
                    setTimeout(() => {
                        // Load Kabupaten
                        window.getWilayah('get-kabupaten', answers.kdprovinsi, 'review_kdkabupaten', answers
                            .kdkabupaten);
                        ...
                        if (answers.kdkabupaten) {
                            setTimeout(() => {
                                ...
                                window.getWilayah('get-kecamatan', answers.kdkabupaten,
                                    'review_kdkecamatan', answers.kdkecamatan);

                                if (answers.kdkecamatan) {
                                    setTimeout(() => {
                                        kecEl.value = answers.kdkecamatan;
                                        window.getWilayah('get-desa', answers.kdkecamatan,
                                            'review_kddesa', answers.kddesa);
            ...
            // ===================================================================
                ...
                html +=
                    `<div><label class="block text-xs font-medium mb-1">Jam Kelahiran</label>
                    <input type="time" name="kelahiran_jamkelahiran" value="${jamValue}" class="w-full border rounded-lg p-2.5"></div>`;

                // 5. Kelahiran Ke-
                html +=
                    `<div><label class="block text-xs font-medium mb-1">Anak Ke-</label>
                ...
                html +=
                    `<div><label class="block text-xs font-medium mb-1">NIK Ibu</label>
                ...
                // 8. NIK Ayah (opsional)
                html +=
                    `<div><label class="block text-xs font-medium mb-1">NIK Ayah</label>
                ...
                questions[currentModul].forEach(q => {
                    ...
                    if (q.type === "select") {
                        input = `<select name="${q.field}" class="w-full border rounded-lg p-2.5 text-sm">
                            <option value="">-- Pilih --</option>
                            ${Object.entries(q.options||{}).map(([k,v]) => ` < option value = "${k}"
                        $ {
                            answers[q.field] === k ? 'selected' : ''
                        } > $ {
                            v
                        } < /option>`).join('')} <
                    /select>`;
                        ...
                        input =
                            `<input type="time" name="${q.field}" value="${val}" class="w-full border rounded-lg p-2.5 text-sm">`;
                    } else {
                        input =
                            `<input type="${q.type==='number'?'number':'text'}" name="${q.field}" value="${answers[q.field]||''}" class="w-full border rounded-lg p-2.5 text-sm">`;
                    }
                    container.innerHTML +=
                        `<div><label class="block text-xs font-medium mb-1">${q.label}</label>${input}</div>`;
                });
            }
        ...
        function drawWaveVisualizer() {
            ...
            for (let i = 0; i < dataArray.length; i++) {
                ...
                const gradient = ctx.createLinearGradient(0, canvas.height / 2 - barHeight / 2, 0, canvas.height / 2 +
                    barHeight / 2);
        ...
        async function validateVoicePrint() {
            ...
            try {
                ...
                if (result.allowed === true) {
                    ...
                    await speak(
                        "Validasi suara berhasil. Sekarang sebutkan nomor Kartu Keluarga 16 digit Anda untuk pendataan."
                        );
                    ...
                    await speak(result.message ||
                        "Maaf, suara Anda sudah terdaftar sebelumnya atau validasi gagal. Pendataan tidak dapat dilanjutkan saat ini."
                        );

                    document.getElementById('voice-status').innerText =
                        'Validasi gagal. Klik mic jika ingin coba lagi, atau hubungi petugas.';
