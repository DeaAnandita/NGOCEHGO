<x-app-layout>
    <div class="flex">
        ...
        <div class="flex-1 py-6 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <h3 class="text-xl font-bold text-gray-800 mb-6">Edit Buku Kader</h3>
                ...
                <form id="formKader" action="{{ route('admin-pembangunan.kader.update', $item->reg) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" id="kode" name="kdkader"
                                value="{{ old('kdkader', $item->kdkader) }}" class="w-full rounded-lg border-gray-300">
                            <p id="kodeError" class="text-red-600 text-xs"></p>
                        </div>

                        <div>
                            <label>Tanggal</label>
                            <input type="date" id="tanggal" name="kader_tanggal"
                                value="{{ old('kader_tanggal', $item->kader_tanggal) }}"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" id="penduduk" name="kdpenduduk"
                                value="{{ old('kdpenduduk', $item->kdpenduduk) }}"
                                class="w-full rounded-lg border-gray-300">
                        ...
                        <div>
                            ...
                            <select id="pendidikan" name="kdpendidikan" class="w-full rounded-lg border-gray-300">
                                ...
                                @foreach ($pendidikan as $p)
                                    <option value="{{ $p->kdpendidikan }}"
                                        {{ old('kdpendidikan', $item->kdpendidikan) == $p->kdpendidikan ? 'selected' : '' }}>
                                        {{ $p->pendidikan }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="bidang" name="kdbidang" class="w-full rounded-lg border-gray-300">
                                <option value="">-- Pilih --</option>
                                @foreach ($bidang as $b)
                                    <option value="{{ $b->kdbidang }}"
                                        {{ old('kdbidang', $item->kdbidang) == $b->kdbidang ? 'selected' : '' }}>
                                        {{ $b->bidang }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="status" name="kdstatuskader" class="w-full rounded-lg border-gray-300">
                                <option value="">-- Pilih --</option>
                                @foreach ($status as $s)
                                    <option value="{{ $s->kdstatuskader }}"
                                        {{ old('kdstatuskader', $item->kdstatuskader) == $s->kdstatuskader ? 'selected' : '' }}>
                                        {{ $s->statuskader }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <textarea name="kader_keterangan" class="w-full rounded-lg border-gray-300">{{ old('kader_keterangan', $item->kader_keterangan) }}</textarea>
                        </div>
    ...
    <script>
        ...
        function validate() {
            ...
            pendudukError.innerText = penduduk.value.length > 0 ? "" : "ID Penduduk wajib";
            ...
            if (!kode.value || !tanggal.value || !penduduk.value || !pendidikan.value || !bidang.value || !status.value) {
                valid = false;
            }
