...
<div class="container">
    ...
    <div class="analysis-box">
        ...
        <table>
            <thead>
                <tr style="background: #1e40af; color: rgb(0, 0, 0);">
    ...
    @if(isset($analysis[$field]))
        ...
        @switch($field)
            ...
            @case('status_kerja')
                @php
                    // Default nilai
                    $interpretasi = "Status kedudukan kerja masyarakat beragam.";
                    $rekom        = "Pemantauan kondisi ketenagakerjaan dan diversifikasi lapangan kerja.";

                    // Kelompok 1-3: Berusaha sendiri / wirausaha (mandiri)
                    if (in_array($kode, [1, 2, 3])) {
                        $interpretasi = "Mayoritas penduduk bekerja sebagai wirausaha atau berusaha mandiri ";
                        if ($kode == 1) {
                            $interpretasi .= "(Berusaha Sendiri - {$persen}%). Kelompok ini menunjukkan semangat kewirausahaan yang tinggi.";
                        } elseif ($kode == 2) {
                            $interpretasi .= "(Berusaha Dibantu Buruh Tidak Tetap - {$persen}%). Terdapat kecenderungan mempekerjakan tenaga bantu secara fleksibel.";
                        } else { // kode 3
                            $interpretasi .= "(Berusaha Dibantu Buruh Tetap - {$persen}%). Usaha sudah lebih stabil dengan tenaga kerja tetap.";
                        }
                        $rekom = "Pelatihan kewirausahaan lanjutan, akses modal mikro/KUR, diversifikasi usaha, serta penguatan manajemen UMKM.";
                    }

                    // Kelompok 4-5: Pekerja formal (gaji tetap)
                    elseif (in_array($kode, [4, 5])) {
                        $interpretasi = "Mayoritas penduduk bekerja di sektor formal dengan penghasilan tetap ";
                        if ($kode == 4) {
                            $interpretasi .= "(Buruh/Karyawan/Pegawai Swasta - {$persen}%). Dominasi tenaga kerja di sektor swasta.";
                        } else { // kode 5
                            $interpretasi .= "(PNS/TNI/POLRI/BUMN/BUMD/Anggota Legislatif - {$persen}%). Banyak yang bekerja di instansi pemerintah atau BUMN.";
                        }
                        $rekom = "Peningkatan kompetensi dan sertifikasi profesi, program jaminan sosial ketenagakerjaan (BPJS Ketenagakerjaan), serta pengembangan karier.";
                    }

                    // Kelompok 6-8: Pekerja bebas / informal / tidak dibayar (rentan)
                    elseif (in_array($kode, [6, 7, 8])) {
                        $interpretasi = "Sebagian besar penduduk termasuk kelompok pekerja rentan ";
                        if ($kode == 6) {
                            $interpretasi .= "(Pekerja Bebas Pertanian - {$persen}%). Banyak yang bekerja sebagai pekerja lepas di sektor pertanian.";
                        } elseif ($kode == 7) {
                            $interpretasi .= "(Pekerja Bebas Non Pertanian - {$persen}%). Dominan pekerja lepas di luar sektor pertanian (misalnya ojek, pedagang kaki lima).";
                        } else { // kode 8
                            $interpretasi .= "(Pekerja Keluarga/Tidak Dibayar - {$persen}%). Tingginya pekerja keluarga yang tidak menerima upah, menunjukkan kerentanan ekonomi.";
                        }
                        $rekom = "Program jaminan sosial bagi pekerja informal, pelatihan keterampilan kerja, akses modal usaha kecil, serta fasilitasi BPJS Ketenagakerjaan mandiri.";
                    }

                    // Kode 9: Belum/Tidak Bekerja
                    elseif ($kode == 9) {
                        $interpretasi = "Mayoritas penduduk dalam kategori Belum/Tidak Bekerja ({$persen}%). Kelompok ini mencakup pengangguran atau yang belum memasuki pasar kerja (misalnya pelajar, lansia, ibu rumah tangga yang tidak mencari kerja).";
                        $rekom = "Program padat karya, pelatihan kerja dan kewirausahaan, bimbingan karier, serta penyaluran tenaga kerja lokal melalui balai latihan kerja.";
                    }

                    // Jika kode tidak terdeteksi (fallback)
                    else {
                        $interpretasi = "Status kedudukan kerja dominan: {$nama} ({$persen}%). Data menunjukkan variasi yang cukup beragam.";
                        $rekom = "Pendataan ulang status ketenagakerjaan dan pemantauan rutin kondisi ekonomi rumah tangga.";
                    }
                @endphp
                @break
            ...
            {{-- Default jika field tidak memiliki mapping khusus --}}
            @default
                @php
...
@endforeach
    ...
    <div class="footer">
        Laporan ini dihasilkan otomatis berdasarkan data terkini.
    </div>
</div>
</body>
</html>