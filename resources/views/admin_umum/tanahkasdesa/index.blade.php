            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                    ...
                    <div class="flex items-center gap-3">
                        <h3 class="text-xl font-bold text-gray-800">Buku Tanah Kas Desa</h3>
                        ...
                        <div x-data="{ open: false }" class="relative">
                            ...
                            <div x-show="open" @click.away="open=false" x-transition
                                ...
                                <a href="{{ route('admin-umum.tanahkasdesa.export', ['search' => request('search')]) }}"
                                    class="block px-4 py-2 text-sm hover:bg-gray-100">
                                    Export Excel
                                </a>

                                <a href="{{ route('admin-umum.tanahkasdesa.exportPdf', ['search' => request('search')]) }}"
                                    class="block px-4 py-2 text-sm hover:bg-gray-100">
                                    Export PDF
                                </a>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <form method="GET" class="flex gap-2">
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Cari Kode / Lokasi..." class="border rounded-lg px-3 py-2 text-sm">
                        ...
                        <a href="{{ route('admin-umum.tanahkasdesa.create') }}"
                        ...
                        <tbody class="text-center">
                            @forelse($data as $d)
                                <tr class="border-t hover:bg-gray-50">
                                    ...
                                    <td>{{ $d->kdtanahkasdesa }}</td>
                                    <td>{{ $d->lokasitanahkasdesa }}</td>
                                    <td>{{ $d->luastanahkasdesa }}</td>
                                    <td>{{ $d->jenis->jenistkd ?? '-' }}</td>

                                    <td>
                                        <div class="flex justify-center gap-2">

                                            <a href="{{ route('admin-umum.tanahkasdesa.show', $d->kdtanahkasdesa) }}"
                                                class="bg-indigo-600 text-white p-2 rounded-lg">
                                                <x-heroicon-o-eye class="w-4 h-4" />
                                            </a>

                                            <a href="{{ route('admin-umum.tanahkasdesa.edit', $d->kdtanahkasdesa) }}"
                                                class="bg-blue-600 text-white p-2 rounded-lg">
                                                <x-heroicon-o-pencil-square class="w-4 h-4" />
                                            </a>

                                            <form
                                                action="{{ route('admin-umum.tanahkasdesa.destroy', $d->kdtanahkasdesa) }}"
                                                method="POST" x-data
                                                @submit.prevent="$dispatch('open-delete-modal',{form:$el,name:'{{ $d->kdtanahkasdesa }}'})">
                                                @csrf
                                                @method('DELETE')
