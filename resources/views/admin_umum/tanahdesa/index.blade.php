<x-app-layout>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
                    ...
                    <div class="flex items-center gap-3">
                        <h3 class="text-xl font-bold text-gray-800">Buku Tanah Desa</h3>
                        ...
                        <div x-data="{ open: false }" class="relative">
                            ...
                            <div x-show="open" @click.away="open=false" x-transition
                                ...
                                <a href="{{ route('admin-umum.tanahdesa.export', ['search' => request('search')]) }}"
                                    class="block px-4 py-2 text-sm hover:bg-gray-100">
                                    📗 Export Excel
                                </a>

                                <a href="{{ route('admin-umum.tanahdesa.exportPdf', ['search' => request('search')]) }}"
                                    class="block px-4 py-2 text-sm hover:bg-gray-100">
                                    📕 Export PDF
                                </a>
                    ...
                    <div class="flex flex-wrap gap-2">
                        <form method="GET" class="flex gap-2">
                            <input type="text" name="search" value="{{ request('search') }}"
                                placeholder="Cari Kode / Pemilik..." class="border rounded-lg px-3 py-2 text-sm">
                        ...
                        <a href="{{ route('admin-umum.tanahdesa.create') }}"
                        ...
                        <tbody class="text-center">
                            @forelse($data as $d)
                                <tr class="border-t hover:bg-gray-50">
                                    ...
                                    <td>{{ $d->kdtanahdesa }}</td>
                                    <td>{{ $d->pemiliktanahdesa }}</td>
                                    <td>{{ $d->luastanahdesa }}</td>
                                    <td>{{ $d->statusHak->statushaktanah ?? '-' }}</td>

                                    <td>
                                        <div class="flex justify-center gap-2">
                                            <a href="{{ route('admin-umum.tanahdesa.show', $d->id) }}"
                                                class="bg-indigo-600 text-white p-2 rounded-lg">
                                                <x-heroicon-o-eye class="w-4 h-4" />
                                            </a>

                                            <a href="{{ route('admin-umum.tanahdesa.edit', $d->id) }}"
                                                class="bg-blue-600 text-white p-2 rounded-lg">
                                                <x-heroicon-o-pencil-square class="w-4 h-4" />
                                            </a>

                                            <form action="{{ route('admin-umum.tanahdesa.destroy', $d->id) }}"
                                                method="POST" x-data
                                                @submit.prevent="$dispatch('open-delete-modal',{form:$el,name:'{{ $d->kdtanahdesa }}'})">
                                                @csrf
                                                @method('DELETE')
    ...
    <div x-data="{ open: false, form: null, name: '' }"
        ...
        <div class="bg-white p-6 rounded-xl w-full max-w-md">
            ...
            <p class="mb-6">Hapus tanah <span class="font-semibold text-red-600" x-text="name"></span>?</p>
            <div class="flex justify-end gap-3">
                <button @click="open=false" class="px-4 py-2 bg-gray-200 rounded-lg">Batal</button>
                <button @click="form.submit()" class="px-4 py-2 bg-red-600 text-white rounded-lg">Hapus</button>
            </div>
        </div>
    </div>
</x-app-layout>
