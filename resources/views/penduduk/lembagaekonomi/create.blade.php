    ...
    <div class="flex">
        ...
        <div class="flex-1 py-6 px-4 sm:px-6 lg:px-8">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-lg font-bold text-gray-800 mb-6">Tambah Data Lembaga Ekonomi</h3>

                <form action="{{ route('penduduk.lembagaekonomi.store') }}" method="POST">
                    @csrf
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach($masterLembaga as $index => $lembaga)
                                <div>
                                    ...
                                    <select name="lemek_{{ $loop->iteration }}"
                                            class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                        <option value="">-- Silahkan Pilih --</option>
                                        @foreach($masterJawabLemek as $jawab)
                                            <option value="{{ $jawab->kdjawablemek }}">{{ $jawab->jawablemek }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            @endforeach
