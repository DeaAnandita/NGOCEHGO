                ...
                <form action="{{ route('penduduk.programserta.update', $programSerta->nik) }}" method="POST">
                    ...
                    <div class="mb-8">
                        ...
                        <select name="nik" id="nik"
                            ...
                            @foreach($penduduks as $p)
                                <option value="{{ $p->nik }}" {{ $programSerta->nik == $p->nik ? 'selected' : '' }}>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach($masterProgramSerta as $index => $program)
                                @php $col = "programserta_" . $program->kdprogramserta; @endphp
                                <div>
                                    ...
                                    <select name="{{ $col }}"
                                        ...
                                        @foreach($masterJawab as $jawab)
                                            <option value="{{ $jawab->kdjawabprogramserta }}"
                                                {{ $programSerta->$col == $jawab->kdjawabprogramserta ? 'selected' : '' }}>
                                                {{ $jawab->jawabprogramserta }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
