    ...
    <div class="flex">
        ...
        <div class="flex-1 py-6 px-4 sm:px-6 lg:px-8 overflow-x-hidden">
            <div class="bg-white rounded-2xl shadow-lg p-6">
                <h3 class="text-xl font-bold text-gray-800 mb-6">Edit Data Usaha ART</h3>

                <form action="{{ route('penduduk.usahaart.update', $usahaart->nik) }}" method="POST">
                    @csrf
                    @method('PUT')

                    <!-- Informasi Usaha ART -->
                    <div class="mb-8">
                        <h4 class="text-sm font-semibold text-gray-700 mb-4">Informasi Usaha Rumah Tangga</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">

                            <!-- NIK -->
                            <div>
                                <label class="block text-sm font-medium text-gray-700">NIK</label>
                                <select id="nik" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500" disabled>
                                    <option value="{{ $usahaart->nik }}">
                                        {{ $usahaart->penduduk->penduduk_namalengkap ?? 'Tidak ditemukan' }} ({{ $usahaart->nik }})
                                    </option>
                                </select>
                                <input type="hidden" name="nik" value="{{ $usahaart->nik }}">
                                @error('nik')
                            ...
                            <div>
                                ...
                                <select name="kdlapanganusaha" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                    ...
                                    @foreach($lapangan_usahas as $item)
                                        <option value="{{ $item->kdlapanganusaha }}" {{ old('kdlapanganusaha', $usahaart->kdlapanganusaha) == $item->kdlapanganusaha ? 'selected' : '' }}>
                                            {{ $item->lapanganusaha }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="number" name="usahaart_jumlahpekerja" value="{{ old('usahaart_jumlahpekerja', $usahaart->usahaart_jumlahpekerja) }}"
                                    class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500" min="0">
                            ...
                            <div>
                                ...
                                <input type="text" name="usahaart_namausaha" value="{{ old('usahaart_namausaha', $usahaart->usahaart_namausaha) }}"
                            ...
                            <div>
                                ...
                                <select name="kdtempatusaha" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                    ...
                                    @foreach($tempat_usahas as $item)
                                        <option value="{{ $item->kdtempatusaha }}" {{ old('kdtempatusaha', $usahaart->kdtempatusaha) == $item->kdtempatusaha ? 'selected' : '' }}>
                                            {{ $item->tempatusaha }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select name="kdomsetusaha" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                                    ...
                                    @foreach($omset_usahas as $item)
                                        <option value="{{ $item->kdomsetusaha }}" {{ old('kdomsetusaha', $usahaart->kdomsetusaha) == $item->kdomsetusaha ? 'selected' : '' }}>
                                            {{ $item->omsetusaha }}
                                        </option>
                                    @endforeach
                                </select>
