                    ...
                    <div class="mb-8">
                        ...
                        <select name="nik" id="nik"
                            ...
                            @foreach($penduduks as $p)
                                <option value="{{ $p->nik }}" {{ $lembagaMasyarakat->nik == $p->nik ? 'selected' : '' }}>
                                    {{ $p->nama }} ({{ $p->nik }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach($masterLembaga as $index => $lembaga)
                                @php $col = "lemmas_" . $loop->iteration; @endphp
                                <div>
                                    ...
                                    <select name="{{ $col }}"
                                        ...
                                        @foreach($masterJawabLemmas as $jawab)
                                            <option value="{{ $jawab->kdjawablemmas }}"
                                                {{ $lembagaMasyarakat->$col == $jawab->kdjawablemmas ? 'selected' : '' }}>
                                                {{ $jawab->jawablemmas }}
                                            </option>
                                        @endforeach
                                    </select>
